...
extension HashtagView: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let comment =  self.hashtags[indexPath.row].text
        let index = comment.index(after: comment.startIndex)
        let tag = comment[index...]
        self.delegate?.cellSelected(comment: String(tag))
    }
}
